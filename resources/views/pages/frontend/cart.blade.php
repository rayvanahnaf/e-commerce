                        ...
                        <div class="flex flex-start flex-wrap items-center mb-4 -mx-4" data-row="1">
                            <div class="px-4 flex-none">
                                <div class="" style="width: 90px; height: 90px">
                                    <img src="{{ $row->product->product_galleries()->exists() ? url('storage/product/gallery', $row->product->product_galleries->first()->image) : 'data:image/png;base64, iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN88B8AAsUB4ZtvXtIAAAAASUVORK5CYII=' }}"
                                        alt="{{ $row->product->name }}" class="object-cover rounded-xl w-full h-full" />
                                </div>
                            </div>
                            <div class="px-4 w-auto flex-1 md:w-5/12">
                                <div class="">
                                    ...
                                    <span class="text-sm md:text-lg">{{ $row->product->category->name }}</span>
                                    <h6 class="font-semibold text-base md:text-lg block md:hidden">
                                        IDR {{ number_format($row->product->price) }}
                                    </h6>
                                </div>
                            </div>
                            <div class="px-4 w-auto flex-none md:flex-1 md:w-5/12 hidden md:block">
                                <div class="">
                                    <h6 class="font-semibold text-lg">IDR {{ number_format($row->product->price) }}</h6>
                                </div>
                            </div>
                            <div class="px-4 w-2/12">
                                <div class="text-center">
                                    <form action="{{ route('deletecart', $row->id) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <button type="submit"
                    ...
                    <div class="bg-gray-100 px-4 py-6 md:p-8 md:rounded-3xl">
                        <form action="{{ route('checkout') }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="flex flex-col mb-4">
                                ...
                                <input data-input type="text" id="address" name="addres"
                                    class="border-gray-200 border rounded-lg px-4 py-2 bg-white text-sm focus:border-blue-200 focus:outline-none"
                            ...
                            <div class="flex flex-col mb-4">
                                ...
                                <input data-input type="text" id="phone-number" name="phone"
                                    class="border-gray-200 border rounded-lg px-4 py-2 bg-white text-sm focus:border-blue-200 focus:outline-none"
                                    ...
                                    <div class="px-2 w-6/12 h-24 mb-4">
                                        <button type="button" data-value="dhl" data-name="courier" name="courier"
                                            ...
                                            <img src="{{ asset('frontend/images/content/logo-fedex.svg') }}"
                                                alt="Logo dhl" class="object-contain max-h-full" />
                                        </button>
                                    </div>
                                </div>
                            </div>
